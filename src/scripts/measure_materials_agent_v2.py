...
async def run_all_tests(
    ...
    max_concurrent: int = 3,  # Максимум 3 параллельных запросов
) -> list[TestResult]:
    """
    ...
    Args:
        ...
        delay_between_tests: Unused (kept for compatibility)
    ...
    logger.info(f"🚀 Starting {len(test_topics)} tests (max {max_concurrent} concurrent)...")

    # Process in batches
    semaphore = asyncio.Semaphore(max_concurrent)

    async def run_with_semaphore(test_topic: TestTopic) -> TestResult:
        async with semaphore:
            return await run_single_test(client, test_topic)

    # Run all with concurrency limit
    tasks = [run_with_semaphore(topic) for topic in test_topics]
