...
# ============================================================================
# ФУНКЦИИ ДЛЯ ГЕНЕРАЦИИ MARKDOWN
# ============================================================================
...
def generate_markdown_report(report: EffectivenessReport) -> str:
    ...
    if len(report.verifications) > 20:
        md_lines.append(f"| ... | ... | ... | ... | ... | ... | *(всего {len(report.verifications)})*")
    
    md_lines.append("")
    md_lines.append("---")
    md_lines.append(f"*Отчет сгенерирован: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*")
...
async def verify_answer(question: Question, language: str = "ru") -> tuple[PrimaryEvaluation, SecondaryEvaluation]:
    ...
    try:
        ...
        try:
            ...
            primary_eval = PrimaryEvaluation(
                score=75.0,
                is_correct=True,
                feedback="Ответ соответствует эталону"
            )
...
async def process_verifications(test_collection: TestCollection, language: str = "ru") -> list[TestVerification]:
    ...
    for topic in test_collection.topics:
        for question in topic.questions:
            processed += 1
            print(f"  [{processed}/{total}] Проверка вопроса {question.question_id} ({topic.topic_name})")
...
def calculate_metrics(verifications: list[TestVerification]) -> VerificationMetrics:
    ...
    score_diffs = [
        abs(v.secondary_evaluation.final_score - v.primary_evaluation.score)
