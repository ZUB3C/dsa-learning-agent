...
### Пример использования API v2

```bash
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{
    "topic": "Тип данных куча",
    "user_level": "beginner",
    "user_id": "test_user",
    "language": "ru"
  }' \
  http://localhost:8000/api/v2/materials/generate
```
...
Вначале надо:
1. Скачать файл [algobook.pdf](https://www.babichev.org/books/AlgoBook.pdf)
2. Создать папку `data/knowledge_base`
3. Скопировать `algobook.pdf` в папку `data/knowledge_base`
...
```bash
# Seed knowledge base from data/knowledge_base directory
uv run python -m src.scripts.seed_knowledge_base data/knowledge_base

# Build TF-IDF corpus for Adaptive RAG (optional)
uv run python -m src.scripts.build_tfidf_corpus
```
...
| Метод  | Эндпоинт                             | Описание                                     |
| ------ | ------------------------------------ | -------------------------------------------- |
| `POST` | `/api/v2/materials/generate`         | Генерация материала через ToT Orchestrator   |
| `GET`  | `/api/v2/materials/status/{generation_id}` | Получить статус генерации             |
...
Вот секция "Метрики" для файла README-project.md:

## Метрики

Для оценки эффективности работы агентов реализованы скрипты измерения метрик по каждому компоненту системы.

### Materials Agent v2

Измеряет качество сгенерированных материалов с использованием метрик DeepEval.

**Собираемые метрики:**
- **Completeness (Полнота)** - степень покрытия темы материалом (0.0-1.0)
- **Relevance (Релевантность)** - соответствие материала запрошенной теме (0.0-1.0)
- **Quality (Качество)** - структурное качество и связность текста (0.0-1.0)
- **ToT Metrics** - количество итераций, исследованных узлов, собранных документов
- **Performance** - время генерации, количество вызовов LLM (GigaChat-2-Max, GigaChat3)
- **Cost** - оценочная стоимость генерации в USD
...
**Результаты:**
- `material-examples/results/materials_agent_v2_evaluation_report.md` - отчет в Markdown
- `material-examples/results/materials_agent_v2_evaluation_results.json` - полные результаты в JSON
- `material-examples/results/{topic_id}_result.json` - индивидуальные результаты по темам

**Подробное описание метрик:** [material-examples/README.md](material-examples/README.md)

---

### Test Generation Agent

Измеряет качество сгенерированных тестовых вопросов с использованием Human Eval (оценка человеком).

**Собираемые метрики:**
- **Accuracy** - процент правильно составленных вопросов (оценка корректности эталонного ответа и ключевых точек)

**Результаты оценки:**
- Всего сгенерировано вопросов: 75
- Некорректных вопросов: 5
- **Accuracy: 93.33%**


**Запуск:**
```
uv run python -m src.scripts.measure_test_generation
```

**Результаты:**
- `test_data.json` - полный набор сгенерированных тестовых вопросов с метаданными для последующей человеческой оценки
- `generated-test-metrics.md` - детальный разбор некорректных вопросов с анализом ошибок

---

### Secondary Verification Agent

Измеряет эффективность вторичной проверки ответов с использованием LLM-судьи.

**Собираемые метрики:**
- **Agreement Rate** - процент согласия первичной и вторичной проверки
- **Primary Accuracy** - точность первичной проверки относительно эталона
- **Secondary Accuracy** - точность вторичной проверки (Judge) относительно эталона
- **Improvement Rate** - улучшение точности от применения вторичной проверки
- **Confusion Matrix** - True Positive, True Negative, False Positive, False Negative
- **False Positive Rate** - доля ложноположительных срабатываний

**Запуск:**
```
uv run python -m src.scripts.measure_secondary_verification \
    --test-data test_data.json \
    --output secondary-verification-report.md
```

**Результаты:**
- `secondary-verification-report.md` - отчет об эффективности вторичной проверки с детальным анализом

---

### Orchestrator Agent

Измеряет точность классификации запросов и производительность оркестратора.

**Собираемые метрики:**
- **Classification Accuracy** - точность определения типа задачи (materials, test, verification, support)
- **Success Rate** - процент успешно обработанных запросов
- **Partial/Error Rates** - процент частично выполненных и ошибочных запросов
- **Response Time** - среднее время ответа и 95-й перцентиль (мс)
- **Support Session Success Rate** - процент успешно сохраненных сессий поддержки

**Запуск:**
```
uv run python -m src.scripts.measure_orchestrator_metrics
```

**Результаты:**
- Метрики выводятся в консоль в формате отчета
- Для сохранения результатов можно перенаправить вывод: `> orchestrator_metrics.md`

### Database Metrics _(в разработке)_
...
### Analytics API _(в разработке)_

**Эндпоинт:** `/api/v1/analytics/generation-stats`

_(Документация в разработке)_
...
## 🏗️ Архитектура Materials Agent v2
